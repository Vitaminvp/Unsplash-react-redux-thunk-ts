...
class Grid extends React.Component<Props> {
  ...
  private handleSort() {
    ...
    const sorting = (
      a: { alt_description: string },
      b: { alt_description: string }
    ): number => (a.alt_description > b.alt_description ? 1 : -1);
    const handleSorting: {
      [sortingParam: string]: (
        a: { alt_description: string },
        b: { alt_description: string }
    ...
    return (
      <>
        ...
        {isButtonVisible ? (
          ...
          >
            Show more {total ? `(${items.length} of ${total})` : ""}
          </Button>
        ) : null}
      </>
    );
